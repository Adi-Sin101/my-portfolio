    ...
    public partial class Experience : System.Web.UI.Page
    {
        ...
        private void LoadExperience()
        {
            using (SqlConnection con = new SqlConnection(cs))
            {
                try
                {
                    ...
                    // Add Duration column for display (calculated from dates)
                ...
                {
                    ShowMessage("Error loading experience: " + ex.Message, "danger");
                }
            }
        }
        ...
        protected void btnAddExperience_Click(object sender, EventArgs e)
        {
            if (Page.IsValid)
            {
                ...
                {
                    string uploadedPath = HandleImageUpload();
                ...
                using (SqlConnection con = new SqlConnection(cs))
                {
                    try
                    {
                        ...
                        {
                            ShowMessage("Failed to add experience!", "danger");
                        }
                    }
                    catch (Exception ex)
                    {
        ...
        {
            if (Page.IsValid && !string.IsNullOrEmpty(hdnExperienceId.Value))
            {
                int experienceId = Convert.ToInt32(hdnExperienceId.Value);
                string title = txtTitle.Text.Trim();
                string company = txtCompany.Text.Trim();
                string description = txtDescription.Text.Trim();
                string startDate = txtStartDate.Text.Trim();
                ...
                if (FileUpload1.HasFile)
                {
                    ...
                    {
                        // Delete old image
                        if (!string.IsNullOrEmpty(imagePath) && imagePath != "default-experience.png")
                        {
                ...
                using (SqlConnection con = new SqlConnection(cs))
                {
                    try
                    {
                        ...
                        {
                            ShowMessage("Failed to update experience!", "danger");
                        }
                    }
                    catch (Exception ex)
                    {
        ...
        private void LoadExperienceForEdit(int experienceId)
        {
            using (SqlConnection con = new SqlConnection(cs))
            {
                try
                ...
                {
                    ShowMessage("Error loading experience for edit: " + ex.Message, "danger");
                }
            }
        }
        ...
        private void DeleteExperience(int experienceId)
        {
            using (SqlConnection con = new SqlConnection(cs))
            {
                try
                {
                    ...
                    string getImageQuery = "SELECT ImagePath FROM Experience WHERE Id = @id";
                    SqlCommand getCmd = new SqlCommand(getImageQuery, con);
                    getCmd.Parameters.AddWithValue("@id", experienceId);
                    string imagePath = getCmd.ExecuteScalar()?.ToString();
                    
                    // Hard delete since we don't have IsActive column
                    string query = "DELETE FROM Experience WHERE Id = @id";
                    SqlCommand cmd = new SqlCommand(query, con);
                    cmd.Parameters.AddWithValue("@id", experienceId);
                    ...
                    {
                        ShowMessage("Failed to delete experience!", "danger");
                    }
                }
                catch (Exception ex)
                {
        ...
        private string HandleImageUpload()
        {
            try
            {
                ...
                if (fileExtension == ".jpg" || fileExtension == ".jpeg" || fileExtension == ".png" || fileExtension == ".gif")
                {
                    ...
                    string uniqueFileName = "exp_" + Guid.NewGuid().ToString() + fileExtension;
        ...
        private void SwitchToEditMode()
        {
            ...
            btnUpdateExperience.Visible = true;
            btnCancelEdit.Visible = true;
        }

        private void SwitchToAddMode()
        {
            formTitle.InnerText = "Add New Experience";
            btnAddExperience.Visible = true;
            btnUpdateExperience.Visible = false;
